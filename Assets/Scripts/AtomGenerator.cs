...
public class AtomGenerator : MonoBehaviour
{
    ...
    public float baseRadius = 1.0f; // Base radius for scaling orbits
    public float updateInterval = 1.0f; // Time interval for updating electron positions
    ...
    private Dictionary<int, string> electronConfigurations = new Dictionary<int, string>
    {
        ...
        { 17, "1s2 2s2 2p6 3s2 3p5" }, { 18, "1s2 2s2 2p6 3s2 3p6" }, { 19, "1s2 2s2 2p6 3s2 3p6 4s1" }, { 20, "1s2 2s2 2p6 3s2 3p6 4s2" },
    ...
    void Start()
    {
        ...
        InvokeRepeating("UpdateElectronPositions", updateInterval, updateInterval);
    }
    ...
    void PlaceElectrons(string configuration)
    {
        ...
        {
            int shell = orbital[0] - '0'; // 1, 2, 3, etc.
            char subshell = orbital[1]; // s, p, d
            int electronCount = int.Parse(orbital.Substring(2)); // number of electrons

            switch (subshell)
            {
                case 's':
                    PlaceElectronsInSOrbital(shell, electronCount);
                    break;
                case 'p':
                    PlaceElectronsInPOrbital(shell, electronCount);
                    break;
                case 'd':
                    PlaceElectronsInDOrbital(shell, electronCount);
                    break;
            }
        }
    }
    ...
    {
        float orbitalRadius = shell * baseRadius; // Adjust the multiplier to make the orbit smaller
    ...
    {
        float orbitalAmplitude = shell * baseRadius; // Adjust the multiplier to make the orbit smaller
    ...
    {
        float orbitalAmplitude = (shell - 1) * baseRadius + 0.3f; // 3d orbitals are closer to 4s
        Vector3[] axes = { Vector3.right, Vector3.up, Vector3.forward, 
    ...
    // Method to update electron positions periodically
    void UpdateElectronPositions()
    {
        foreach (GameObject electron in electrons)
        {
            ...
            if (orbitalScript != null)
            {
                // Randomize the position within the sphere of the orbital's amplitude
                electron.transform.position = nucleus.position + Random.insideUnitSphere * orbitalScript.amplitude;
            }
        }
    }
}
